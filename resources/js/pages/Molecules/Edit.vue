...
import type { MoleculeItem } from './DeleteMoleculeModal.vue';
...
const form = useForm({
    ...
    bond_type: props.molecule.bond_type,
    description: props.molecule.description,
    model_3d_file: null as File | null,
});

const submit = () => {
    // PHP doesn't populate $_FILES on native PUT requests; use POST with method spoofing
    form.transform((data) => ({
        ...data,
        _method: 'put',
    })).post(route('molecules.update', props.molecule.id_molecule), {
        preserveScroll: true,
        forceFormData: true,
    });
};
...
<template>
    <Head :title="`Ubah Molekul: ${molecule.name}`" />
            ...
            >
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        >
                            Ubah Data Molekul: {{ molecule.name }}
                        </h1>
                        <p class="mt-0.5 text-xs text-slate-400">
                            Perbarui parameter geometri, rumus kimia, atau ganti
                            berkas model 3D AR molekul.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                >
                    ...
                    <FileUploadZone
                        ...
                        :existing-url="molecule.model_3d_url"
                        accept=".glb,.gltf,.obj"
                        :max-size-mb="50"
                        label="Berkas Model 3D (.GLB, .GLTF, .OBJ)"
                        description="Pilih berkas baru jika ingin mengganti model 3D di S3. Jika dibiarkan kosong, berkas lama tetap digunakan."
                        :required="false"
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    >
                        ...
                        <span>Simpan Perubahan Molekul</span>
                    </button>
                </div>
            </form>
        </div>
