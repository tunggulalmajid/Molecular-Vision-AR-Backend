...
const deleteMolecule = () => {
    ...
    form.delete(route('molecules.destroy', props.molecule.id_molecule), {
        preserveScroll: true,
        onSuccess: () => {
            emit('close');
        },
...
<template>
    <ModalDialog
        :show="show"
        title="Konfirmasi Hapus Molekul"
        max-width="md"
        @close="$emit('close')"
    >
        <div class="space-y-4">
            ...
            >
                ...
                <div class="text-sm text-slate-300">
                    ...
                    <p class="mt-0.5 text-xs text-slate-300">
                        Apakah Anda yakin ingin menghapus data molekul
                        <span class="font-bold text-rose-300">{{
                            molecule?.name
                        }}</span>
                        <span v-if="molecule?.formula" class="text-slate-400">
                            ({{ molecule?.formula }})</span
                        >?
                    </p>
                </div>
            </div>
            ...
            <p class="text-xs text-slate-400">
                Data molekul dan visualisasi 3D AR ini tidak akan dapat diakses
                lagi oleh siswa pada modul eksplorasi molekul.
            </p>
